...
class DatabaseService {
    static async saveState(ports, displayCount, extraFieldDefs = []) {
        try {
            ...
            this.updateLastSaveTime(data.lastSave);
            return true;
        } catch {
            return false;
        }
    ...
    static exportJson() {
        const data = {
            ...
            displayCount: AppState.displayCount,
            extraFieldDefs: AppState.extraFieldDefs,
        ...
        a.href = url;
        a.download = `odf_backup_${ODFContext.region}_${ODFContext.sub}_${new Date().toISOString().split('T')[0]}.json`
            .replace(/\s+/g, '_');
    ...
    static parseExcel(buffer) {
        ...
        const extraFieldDefs = this.extractExtraFieldDefs(headerList);
...
const AppState = {
    ...
    async init() {
        ...
        if (saved && Array.isArray(saved.ports) && saved.ports.length > 0) {
            ...
            const defs = defsFromSave ? defsFromSave : this.deriveExtraFieldDefs(saved.ports);
    ...
    deriveExtraFieldDefs(ports) {
        ...
        const candidate = list.find((p) => Array.isArray(p.extraFields) && p.extraFields.length > 0);
        if (candidate) {
            return candidate.extraFields.map((field, index) => {
                const label = field && typeof field.label === 'string' ? field.label.trim() : '';
                return label || `Field ${index + 1}`;
            });
        }

        const valueCarrier = list.find((p) => Array.isArray(p.extraFieldValues) && p.extraFieldValues.length > 0);
        if (valueCarrier) {
            return valueCarrier.extraFieldValues.map((_, index) => `Field ${index + 1}`);
        }

        const mapCarrier = list.find((p) => p && typeof p.customFields === 'object' && !Array.isArray(p.customFields));
        if (!mapCarrier) return [];
        return Object.keys(mapCarrier.customFields);
    }
};
        ...
        container.innerHTML = `
            <div class="port-details">
                ...
                <div class="detail-row">
                    ...
                    <div class="detail-value">
                        ${isEditing ? 
                            `<input type="date" id="dateInput" value="${port.lastMaintained}">` :
                            `<span class="readonly-text">${port.lastMaintained}</span>`
                        }
                    </div>
                </div>
        ...
        if (isEditing) {
            ...
            switchSaveBtn.addEventListener('click', async () => {
                ...
                if (ok) {
                    ...
                    if (!saved) {
                        this.showMessage('Switch completed, but failed to save to data.json.', 'info');
                        return;
                    }
                    this.renderPortGrid();
                    this.renderPortDetails();
    ...
    static initEventListeners() {
                ...
                if (file) {
                    try {
                        ...
                        if (data && Array.isArray(data.ports)) {
                            const defs = Array.isArray(data.extraFieldDefs) ? data.extraFieldDefs : AppState.deriveExtraFieldDefs(data.ports);
                            AppState.extraFieldDefs = defs;
                            ...
                            await DatabaseService.saveState(AppState.ports, AppState.displayCount, AppState.extraFieldDefs);
                        }
                    await AppState.init();
                    this.renderPortGrid();
                    this.renderPortDetails();
                    ...
                    this.showMessage('Error importing file!', 'info');
                }
            }
        ...
        document.getElementById('saveBtn').addEventListener('click', () => {
            (async () => {
                ...
                this.showMessage(ok ? 'All data saved to JSON storage!' : 'Save failed!', ok ? 'success' : 'info');
            })();
        });
